...
import com.liuyang19900520.laymanmall.product.feign.SearchFeignService;
...
public class SpuInfoServiceImpl extends ServiceImpl<SpuInfoDao, SpuInfoEntity> implements
  ...
  public void up(Long spuId) {
    ...
    List<ProductAttrValueEntity> productAttrValueEntities = attrValueService.baseAttrListForSpu(
      spuId);
    ...
    try {
      List<Long> skuInfoSkuIds = skuInfoEntities.stream().map(SkuInfoEntity::getSkuId)
        .collect(Collectors.toList());
    ...
    if (statusUp.getCode() == 0) {
      ...
      log.error(String.valueOf(statusUp.getCode()));
